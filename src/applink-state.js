// What Iris is allowed to know about Veil.
//
...
function describeState({ state, transcript, settings, sttDisabled, shortcuts, windowAlive }) {
  ...
  return {
    ...
    // The single most useful field here. When this is true Veil looks alive and
    ...
    // the user presses the key, nothing happens, and Veil never knew.
    shortcuts: shortcuts || {},
 ...
 *
 * On the Node transport Veil cannot read the peer's credentials, so the caller's
...
function consentCopy(request) {
  ...
  return {
    trusted,
    message: action ? `${who} wants to control Veil.` : `${who} wants to see what Veil is doing.`,
    detail:
      (action
        ? 'It would be able to start and stop listening. '
        : 'It would be able to read Veil’s status, recent warnings and errors — never your transcript, your résumé or your API keys. ') +
      (trusted
        ? 'Its code signature has been verified.'
        : 'Veil cannot verify what this program really is; anything running under your account could make the same claim.') +
      '\n\nYou can change this later in Veil’s settings.',
