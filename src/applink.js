// The publik app link: how Iris asks Veil what it is doing.
//
// Before this, an assistant helping someone with Veil could only see what was on
...
//
// So Veil answers questions instead. Nothing is exposed until the user says yes,
...
/**
 * Ask inside Veil's own window.
 *
 * The first version of this used dialog.showMessageBox, and it was unusable:
 * Veil calls app.dock.hide(), so it is an accessory application and never
 ...
 * bringing it forward.
 *
 * Veil's own window has none of that problem, matches the rest of the app, and
...
function askInWindow(win, copy, scope) {
  ...
  return new Promise((resolve) => {
    ...
    // Veil deliberately never steals focus — except here. A question about who
 ...
 *   `get_state` reflects the moment it is asked rather than the moment Veil
 *   started.
 */
function startAppLink(deps) {
  ...
  link = new AppLinkServer({
    appId: 'com.veil.overlay', // Matches electron-builder.cjs. Read off a build, not guessed.
    appSlug: 'veil',
    appName: 'Veil',
  ...
  link.start().catch((error) => {
    // A link that will not start must never stop Veil from starting. The app
    // worked without this yesterday.
    console.log('[veil] app link unavailable:', error && error.message);
    link = null;
  });
